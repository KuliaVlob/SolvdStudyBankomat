...
public class Validation {
	...
	public void jsonDataValidate(Transaction transaction) {
		...
		if (transaction.getBanknote() > transaction.getAmmount()) {
			...
			System.out.println("Banknote more than the amount you want" + " to withdraw from the account");
			getValidationInfo();
		}
		...
		if (transaction.getAmmount() % minBanknote != 0) {
			...
			System.out.println(
					"To withdraw money from the account," + " the amount must be an integer and a multiple of 10");
			getValidationInfo();
		}
